...
export default function ReportsPage() {
  const { tools, cabinets, drawers, toolTypes, statuses, movements, suppliers } = useDataStore();
  ...
  const cabinetStats = cabinets.map((cabinet) => {
    ...
    return { ...cabinet, toolTypes: cabinetTools.length, totalQuantity, lowStock };
  });
  ...
  const typeStats = toolTypes.filter((t) => t.isActive).map((type) => {
    ...
    return { ...type, toolCount: typeTools.length, totalQuantity };
  });

  // --- Reform tracking from movements ---
  const reformItems = useMemo(() => {
    ...
    return sendMovements.map(send => {
      ...
      // Check if there's a matching return
      const returns = returnMovements.filter(r => r.toolId === send.toolId);
      const totalReturned = returns.reduce((acc, r) => acc + r.quantity, 0);
      const totalSent = sendMovements.filter(s => s.toolId === send.toolId).reduce((acc, s) => acc + s.quantity, 0);
  ...
  // --- Consumption from movements (filtered by date) ---
  ...
  const movementStats = useMemo(() => {
    ...
    const totalIn = entries.reduce((a, m) => a + m.quantity, 0);
    const totalOut = exits.reduce((a, m) => a + m.quantity, 0);
    return { totalIn, totalOut, entries: entries.length, exits: exits.length, total: filteredMovements.length };
  }, [filteredMovements]);
                      ...
                      {cabinetStats.map((cab) => (
                        <TableRow key={cab.id}>
                          ...
                          <TableCell className="text-center">
                            {cab.lowStock > 0 ? (
                              <Badge variant="destructive" className="gap-1"><AlertTriangle className="h-3 w-3" />{cab.lowStock}</Badge>
                      ...
                      <TableBody>
                        {lowStockTools.map((tool) => (
                          <TableRow key={tool.id}>
                            ...
                            <TableCell className="text-center">
                              <Badge variant="destructive">{Math.max(0, tool.minStock - tool.quantity)}</Badge>
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                ...
                <CardContent className="flex items-center gap-4 p-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      {reformOverdue.length} reforma(s) com prazo atrasado!
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Ferramentas que ultrapassaram a data estimada de retorno.
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="bg-card border-border">
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {reformPending.length + reformOverdue.length} pendente(s) | {reformOverdue.length} atrasada(s) | {reformReturned.length} concluida(s)
                  </CardDescription>
                </div>
                <Button variant="outline" onClick={() => {
                            ...
                            <TableCell>
                              {item.estimatedReturn ? (
                                <span className={item.isOverdue ? "text-destructive font-medium" : ""}>
                                  {formatDate(item.estimatedReturn)}
                                </span>
                              ) : (
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                  ...
                  <Button variant="outline" onClick={() => {
                    exportCSV(filteredMovements.map(m => {
                      ...
                      return {
                        Data: formatDateTime(m.date), Tipo: m.type === "entry" ? "Entrada" : m.type === "exit" ? "Saida" : m.type === "reform_send" ? "Reforma (envio)" : m.type === "reform_return" ? "Reforma (retorno)" : "Nota Fiscal",
                ...
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {typeStats.slice(0, 3).map(ts => (
                        <Badge key={ts.id} variant="secondary" className="text-xs">{ts.name}: {ts.totalQuantity}</Badge>
                      ))}
                    </div>
                  </div>
                </CardContent>
